{ pkgs, lib, config, plover-src, plover-stroke-src, rtf-tokenize-src, plover-plugins-manager-src, ... }:
let
  ...
  plugins-manager = with pkgs.python3Packages; buildPythonPackage rec {
    ...
    src = plover-plugins-manager-src;
...
in
{
  home.packages = [ plover-with-plugins ];
}
